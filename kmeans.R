# Script created by Matt Peeples: http://www.public.asu.edu/~mpeeple/
...
choose.per <- function(){
	readline("Covert data to percents? 1=yes, 2=no : ")
} 
...
choose.stand <- function(){
	readline("Z-score standardize data? 1=yes, 2=no : ")
} 
...
choose.level <- function(){
	readline("How many clustering solutions to test (> row numbers)? ")
} 
...
k.rand <- function(x){
	...
	for (i in 2:n.lev) 
		rand.wss[i] <- sum(kmeans(km.rand, centers=i)$withinss)
...
for (i in 1:250) 
	lines(log(rand.mat[,i]),type='l',col='red')
...
for (i in 1:250) 
	lines(rand.mat[,i],type='l',col='red')
...
choose.clust <- function(){
	readline("What clustering solution would you like to use? ")
} 
